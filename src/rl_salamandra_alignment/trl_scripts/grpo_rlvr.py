...
def piecewise_reward(value, mean=MEAN_LENGTH, std_dev=SD_LENGTH):
    """
    Positive reward within 2σ, negative penalty beyond.
    """
    # get z score
    z_abs = abs((value - mean) / std_dev)
    
    if z_abs <= 2.0:
        # Quadratic reward: 1 at mean, 0 at 2σ
        reward = 1.0 - (z_abs / 2.0)**2
    else:
        # Negative penalty that increases with distance
        reward = -0.5 * (z_abs - 2.0)
    # Scale:
    reward = MAX_REWARD * reward
    return reward


def len_reward_func(**kwargs):
    """Reward function that assigns higher scores to completions which are closer to a normal distribution with predetermined mean and SD (in terms of token count, that is why we use `completion_ids`).
    And penalizes those that are far away.
    """
    # print("="*100)
    # print(kwargs)
    # print(type(kwargs))
    ...
    return [piecewise_reward(len(completion_ids)) for completion_ids in completions_ids]

REWARD_FUNCS = [len_reward_func]
